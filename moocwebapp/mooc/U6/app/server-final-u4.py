...
def home():
    ...
    if request.method == 'POST' and str.strip(str(request.form['message'])) != "" \
            and request.form['post_submit'] == 'Post!':
        messages = session['messages']
        if not messages:
            messages = []
        messages.append([time(), request.form['message']])
        save_current_user()
    elif request.method == 'POST' and request.form['post_submit'] == 'Borrar!':
        messages = session['messages']
        for mensaje_borrar in request.form.getlist('messages_post'):
            tmp = mensaje_borrar
            index1 = tmp.index('[')
            index2 = tmp.index(',')
            index3 = tmp.index("'")
            tiempo = tmp[index1+1:index2]
            msj = tmp[index3+1:len(tmp)-2]
            borrar = [(float)(tiempo), msj]
            if borrar in messages:
                messages.remove(borrar)
        save_current_user()
    ...
    return render_template('home.html', logged=True, nickname=session['user_name'], messages=messages,
                           friends=session['friends'], selected_friend=request.form.get('friend', 'All'),
                           total_amigos=len(session['friends']))
...
def profile():
    ...
    """
    if os.path.isfile(os.path.dirname(__file__) + '\\static\\images\\' + session['email'] + '.png') is True:
        foto = session['email']
    else:
        foto = 'anonimo'
    ...
    if 'user_name' not in session:
        return process_error("you must be logged to use the app / debe registrarse antes de usar la aplicacion",
                             url_for("login"))
    if request.method == 'POST' and request.form['accept_submit'] == 'Accept':
    ...
    elif request.method == 'POST' and request.form['accept_submit'] == 'Subir' \
            and secure_filename(request.files['foto'].filename) != '':
        f = request.files['foto']
        cargar = os.path.join(os.path.dirname (__file__), 'static\\images', secure_filename(f.filename))
        f.save(cargar)
        if os.path.isfile(os.path.dirname (__file__) + '\\static\\images\\' + session['email'] + '.png') is True:
            os.remove(os.path.join(os.path.dirname (__file__), 'static\\images', session['email'] + '.png'))
        os.rename(os.path.dirname (__file__) + '\\static\\images\\'+secure_filename(f.filename),
                  os.path.dirname (__file__) + '\\static\\images\\' + session['email'] + '.png')
...
def get_friend_messages():
    ...
    return jsonify([(message[0], message[2]) for message in sorted(friends_messages, key=lambda x: x[1])])
