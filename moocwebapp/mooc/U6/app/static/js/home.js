...
function validateMessageDHTML() {
    ...
    var errores = ""
    var retorno = true
//(msj.value.trim().length == 0) ||
    if (((msj.value.trim().length > 128))) {
...
function updateFriendMessagesJQUERY() {
    $.post("/friend_messages",
        ...
        function (data) {
            $('select[name="messages"]').empty()
            data.forEach(function (item) {
                if ((((item[1]).toLowerCase()).trim()).search((($('input[name="buscar_mensaje"]').val()).toLowerCase()).trim()) != -1) {
                    $('<option>').appendTo('#messages').text(item[0] + ": " + item[1])
                }
            })
            $('#selected_friend').text($('select[name="friend"]').val())
            if (($('input[name="buscar_mensaje"]').val()).trim() == '') {
                $('#filtrar_mensajes').text('Todos los mensajes')
            } else {
                $('#filtrar_mensajes').text($('input[name="buscar_mensaje"]').val())
            }
        }, "json");
}
...
function pulsadoCheck(id) {
    if (document.getElementById("check" + id).checked) {
        ...
        i = seleccionados.findIndex(function (item) {
...
function emoticonJQUERY() {
    if($('select[name="emoticon_post"]').val()!=''){
            $('#post').val($('#post').val() + $('select[name="emoticon_post"]').val())
    }
}